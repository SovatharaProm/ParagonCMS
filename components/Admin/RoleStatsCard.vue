<template>
    <div class="p-4">
        ...
        <v-dialog v-model="createEventDialog" width="50%">
            <v-card flat>
                <v-card-title
                    class="d-flex align-center pe-2 bg-[#2B348F] text-white"
                >
                    Create Roles
                </v-card-title>
                <v-divider></v-divider>
    ...
    try {
        ...
        if (roleId) {
            ...
            if (!membersResponse.ok) {
                ...
                throw new Error(
                    errorData.message || "Failed to fetch members in role"
                );
            }
            ...
            users.value.forEach((user) => {
                user.inRole = membersInRole.some(
...
const deleteRole = async () => {
    try {
        ...
        toast.error("Role deleted successfully", { timeout: 3000 });
    } catch (error) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            try {
                ...
                throw new Error(
                    "Failed to create role. Invalid response from server."
                );
            }
...
const executeSaveRoleChanges = async () => {
    try {
        const usersToAdd = users.value
            .filter(
                (user) => user.inRole && !currentUser.value.includes(user.id)
            )
            .map((user) => user.id);
...
const updateRoles = async (userIds, action) => {
    ...
    if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
            errorData.message || `Failed to ${action} role to users`
        );
    }
