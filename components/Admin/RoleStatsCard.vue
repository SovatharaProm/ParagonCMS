          ...
          <tr v-for="role in filteredRoles" :key="role.id">
            ...
            >
              <div class="flex flex-col md:flex-row justify-end space-y-2 md:space-y-0 md:space-x-2">
                <NuxtLink
                  :to="{ path: '/admin/permission/rolepermission', query: { role_id: role.id } }"
                  class="bg-blue-900 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
                  title="Edit Permission"
                >
                  <Icon name="lucide:edit" class="text-lg"></Icon>
                </NuxtLink>
                <button
                  @click="editRole(role)"
                ...
                >
                  <Icon name="clarity:assign-user-line" class="text-lg"></Icon>
                </button>
                <button
                  ...
                  title="Delete Role"
                >
                  <Icon name="material-symbols:delete" class="text-lg"></Icon>
                </button>
              </div>
            </td>
          </tr>
                ...
                <v-list-item-action>
                  <v-checkbox v-model="usersearch.inRole">
                    <template v-slot:label>
                      <v-list-item-content>
                        <v-list-item-title>{{ usersearch.email }}</v-list-item-title>
                      </v-list-item-content>
                    </template>
                  </v-checkbox>
                </v-list-item-action>
...
const fetchRoles = async () => {
  try {
    ...
    toast.error(error.message || "An unexpected error occurred", { timeout: 3000 });
  }
};
