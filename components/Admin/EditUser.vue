      ...
      <v-row class="flex-wrap mb-6">
        <v-col v-for="role in availableRoles" :key="role.id" class="w-1/2 md:w-1/3 lg:w-1/4 mb-4">
          <label class="flex items-center cursor-pointer">
            <input type="checkbox" v-model="selectedRoles" :value="role.id" class="mr-2" />
...
const fetchUserDetails = async () => {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/edit-user-access?user_id=${userId.value}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
...
const updatePermissions = async () => {
  ...
  try {
    const token = authStore.token;
    console.log('Formatted permissions:', JSON.stringify(formattedPermissions));
    const response = await fetch(`${API_BASE_URL}/update-user-access`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
