<template>
  ...
  <div class="my-5 flex justify-end">
    <button @click="openCreatePageModal(false)" class="my-auto px-5 py-2 bg-blue-900 text-white rounded-md font-medium">
      Create main page
    </button>
  </div>
        ...
        <div :key="request.id" class="bg-white p-5 rounded-lg drop-shadow flex flex-col gap-4">
          <div class="flex justify-between">
            ...
            <div class="flex gap-5 my-auto">
              ...
              <NuxtLink :to="`/builder?id=${request.id}`" class="my-auto px-5 py-2 bg-blue-900 text-white rounded-md font-medium">
                Edit
              </NuxtLink>
          ...
          <div v-if="request.children && request.children.length > 0" class="pl-8 p-5 grid gap-2">
            <NestedChildren
  ...
  <div v-if="updatePageModal" class="fixed mx-auto inset-0 flex justify-center items-center">
    <div class="absolute inset-0 bg-opacity-25 backdrop-blur-sm"></div>
    <div class="relative bg-white p-8 rounded-lg shadow-lg max-w-md w-full z-10">
...
const fetchPages = async () => {
  try {
    ...
    if (data.code === 200) {
      ...
      childSwitchStates.value = data.data.Pages.map(page => initializeChildStates(page.children));
    } else {
...
async function togglePage(page, parentIndex, childIndex = null) {
  ...
  try {
    ...
    if (data.code === 200 || data.code === 201) {
      ...
      if (childIndex !== null) {
        childSwitchStates.value[parentIndex][childIndex].is_active = data.new_state;
      } else {
      ...
      if (childIndex !== null) {
        childSwitchStates.value[parentIndex][childIndex].is_active = originalState;
      } else {
